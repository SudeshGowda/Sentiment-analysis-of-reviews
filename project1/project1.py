...
def perceptron_single_step_update(
    ...
    if label * (current_theta @ feature_vector + current_theta_0) <= 0.001:
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    r, c = np.shape(feature_matrix)
    current_theta = np.zeros(c)
    current_theta_0 = 0
    for t in range(T):
...
def average_perceptron(feature_matrix, labels, T):
    ...
    # Your code here
    r, c = np.shape(feature_matrix)
    current_theta = theta = np.zeros(c)
    current_theta_0 = theta_0 = 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            # Your code here
            current_theta, current_theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], current_theta,
            ...
            theta = np.add(theta, current_theta)
    return theta / (feature_matrix.shape[0] * T), theta_0 / (feature_matrix.shape[0] * T)
    raise NotImplementedError
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(feature_matrix.shape[0]):
        if theta @ feature_matrix[i] + theta_0 > 0.00001:
            pred[i] = 1
        else:
            pred[i] = -1
    return pred
